...
Then /^it exits with a status of (\d+), and a stderr including:$/ do |status, stderr|
  expect(last_cmdline.exitstatus).to eq status.to_i
  expect(last_cmdline.stderr.chomp).to include interpret_curlies(stderr).chomp
end
...
Then 'I see the file "$filename":' do |filename, body|
  in_proving_grounds do
    ...
    body and expect(strip_trailing_whitespace File.read filename).to eq strip_trailing_whitespace(body)
  end
end
